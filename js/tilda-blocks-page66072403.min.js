...
function t1148_init(recid){var rec=document.getElementById('rec'+recid);if(!rec)return;var slider=rec.querySelector('.t1148__slider');if(!slider)return;var slides=Array.from(slider.querySelectorAll('.t1148__item'));if(!slides.length)return;var prevButton=rec.querySelector('.t1148__control_left');var nextButton=rec.querySelector('.t1148__control_right');if(prevButton){prevButton.addEventListener('click',t1148_nextSlide.bind(null,slider,slides,'backwards'))}
if(nextButton){nextButton.addEventListener('click',t1148_nextSlide.bind(null,slider,slides,'forwards'))}
var slideTimeout=parseInt(slider.getAttribute('data-slide-timeout'),10)||0;var timeoutIdObj={timeoutId:null,};var stopSlider=t1148_stopAutoSlides.bind(null,timeoutIdObj);var startSlider=t1148_startAutoSlides.bind(null,slider,slides,slideTimeout,timeoutIdObj);if(slideTimeout){slider.addEventListener('mouseenter',stopSlider);slider.addEventListener('mouseleave',startSlider);slider.addEventListener('mousemove',stopSlider,{once:!0});slider.addEventListener('touchstart',stopSlider);var addButtonListener=function(button,stop,start){button.addEventListener('mouseenter',stop);button.addEventListener('mouseleave',start);button.addEventListener('focus',stop);button.addEventListener('blur',start)}
if(prevButton){addButtonListener(prevButton,stopSlider,startSlider)}
...
slider.addEventListener('wheel',t1148_debounce(t1148_handleWheel.bind(null,slider,slides),300));var playButtons=rec.querySelectorAll('.t1148__play-btn');playButtons.forEach(function(playButton){playButton.addEventListener('click',t1148_handlePlayClick.bind(null,slider,playButton,stopSlider))})}
